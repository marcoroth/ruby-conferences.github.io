...
# result = MeetupClient::Client.query(EventsQuery, variables: { groupId: "rubyonrails-ch" })
#
# events = Array(result.original_hash.dig("data", "groupByUrlname", "unifiedEvents", "edges"))
#
# events.each do |_event|
#   event = OpenStruct.new(_event["node"])
#
#   puts event.title
# end
...
# class MeetupGroup
#   def self.path
#     Pathname.new("./_data/meetup_groups.yml")
#   end
#
#   def self.file
#     YAML.load(path.read)
#   end
#
#   def self.meetupdotcom_groups
#     file.find { |item| item["id"] == "meetupdotcom" }.dig("groups").map { |group| MeetupGroup.new(id: group["id"], name: group["name"]) }
#   end
#
#   def self.luma_groups
#     file.find { |item| item["id"] == "luma" }.dig("groups")
#   end
#
...
#
#     Array(result.original_hash.dig("data", "groupByUrlname", "unifiedEvents", "edges"))
#
#   end
# end
